...
const HeartbeatTableComponent: React.FC<{
  ...
  const getStatusColumn = (title: string, keyAndIndex: string) => {
    return (
      <Column
        ...
        onFilter={(value, record: HeartbeatStatus) =>
  ...
  const getCameraStatusColumn = (title: string, index: number) => {
    return (
      <Column
        title={title}
        key={"camera_status_" + index.toString()}
        align="center"
        render={(value, record: HeartbeatStatus, _) => (
          <StatusCircle
            status={
              record.Camera![index]
                ? record.Camera![index]["status"]
                : undefined
            }
          />
        )}
        ...
        onFilter={(value, record: HeartbeatStatus) =>
          record.Camera![index]["status"] !== undefined &&
  ...
  const getModuleStatusColumn = (title: string, index: number) => {
    return (
      <Column
        title={title}
        key={"module_status_" + index.toString()}
        align="center"
        render={(value, record: HeartbeatStatus, _) => (
          <StatusCircle
            status={
              record.Module![index]
                ? record.Module![index]["status"]
                : undefined
            }
          />
        )}
        ...
        onFilter={(value, record: HeartbeatStatus) =>
          record.Module![index]["status"] !== undefined &&
  ...
  const getTooltipTitle = (record: HeartbeatStatus) => {
    ...
    if (camera != null && camera[0] != undefined) {
      ...
      if (cameraTimestamp != null) {
        output +=
          "Camera : " +
          getMinuteDiffer(
            lastUpdateDatetime,
            new Date(cameraTimestamp!)
          ).toString() +
          " m(s) ago.\n"
      }
    }
    ...
    return (
      <div>
        <Space>
          ...
          <StatusCircle status={Status.ACTIVE} />
  ...
  return (
    ...
    >
      <Column
        ...
        sorter={(a: HeartbeatStatus, b: HeartbeatStatus) =>
